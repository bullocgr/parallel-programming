...
int main( int argc, char *argv[ ] ) {
    ...
    for( int t = 0; t < NUMTRIES; t++ ) {
		...
		for( int n = 0; n < NUMTRIALS; n++ ) {
			...
			//if d is less than 0 then the circle was missed
			if(d >= 0) {
				// hits the circle:
			...
			//if tmin is less than 0 then the circle engulfs the laser pointer
				if(tmin >= 0) {
		...
		printf("Threads: %d      Trials: %d      probability: %f      megaTrialsPerSecond: %f\n", NUMT, NUMTRIALS, currentProb, megaTrialsPerSecond);
	}

	printf("Peak performance: %f\n", maxPerformance);
	printf("avg performance: %f\n", summflops/(double)NUMTRIES);
}
